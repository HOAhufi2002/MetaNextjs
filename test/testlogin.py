    ...
    try:
        ...
        for filename in os.listdir('temp'):
            if filename.endswith("_face_encoding.json"):
                user_id = filename.split('_face_encoding.json')[0]
        ...
        if ret:
            ...
            for (x, y, w, h) in faces:
                ...
                if result and result['user_id']:
                    ...
                    text = f"User ID: {user_id}, {match_percentage:.2f}%"
                    cv2.putText(frame, text, (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 0), 2)
                    
                    if match_percentage > 50:
                        print(f"Match found: User ID {user_id} with {match_percentage:.2f}% match.")
                        cap.release()
...
if __name__ == "__main__":
    try:
        capture_frames_from_webcam()  # Call capture function
    except Exception as e:
        print(f"Lỗi: {str(e)}", file=sys.stderr)
        sys.exit(1)
